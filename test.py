...
class videoPlayer():
	def __init__(self, (windowWidth, windowHeight), fullscreen = False):
		...
		self.player = player.Player(videorenderfunc=self.__texUpdate)
	...
	def load_video(self, vidSource):
		...
		if(self.player.audio):
			print self.player.audio
			self.audiostream = pyaudio.PyAudio().open(
				rate 	= self.player.audio["rate"] ,
				channels  = self.player.audio["channels"],
